...
<link rel="stylesheet" type="text/css" href ="${cpath }/resources/css/mail/receiveList.css">
</head>
<body>
		...
		<div id="mailSideBox">
			...
			<form id="mail_column" method="POST" action="${cpath }/mail/receiveList">
				<div class="mailSearchBox">
					<input type="text" id="mail_receive_inputSearch" name="mail_search" placeholder="검색어를 입력하세요" >
					<input type="submit" name="mail_submit" value="검색">
				</div>
					<select id="mail_receive_select" name="mail_column">
				...
				<table>
		        	...
		        	<tbody>
		        		<c:forEach var="dto" items="${list}">
		        			<c:if test="${dto.mail_read  == 1}">
			    			<tr class="ReadMail">
			        			<td><input class="chk" name="rowcheck" type="checkbox" value="${dto.mail_idx }"></td>
			        			<td><a href="${cpath}/mail/selectone/receiver/${dto.mail_idx}">${dto.mail_title}</a></td>
			        			<td>${dto.mail_receiver }</td>
			        			<td>${dto.mail_writer }</td>
			        			<td>${dto.mail_writedate}</td>
			        		</tr>
			        		</c:if>
			        		<c:if test="${dto.mail_read == 0}">
			        		<tr class="NotReadMail">
			        			<td><input class="chk" name="rowcheck" type="checkbox" value="${dto.mail_idx }"></td>
			        			<td><a href="${cpath}/mail/selectone/receiver/${dto.mail_idx}">${dto.mail_title}</a></td>
			        			<td>${dto.mail_receiver }</td>
			        			<td>${dto.mail_writer }</td>
			...
			<div id="mail_foot">
				<div id="mail_paging">
					<c:if test="${mailpaging.prev }">
						<a href="${cpath }/mail/receiveList?page=${mailpaging.begin - mailpaging.perPage}&mail_search=${mailpaging.mail_search}&mail_column=${mailpaging.mail_column}">[이전]</a>
			    	</c:if>
					...
					<c:set var="mail_search" value="${mailpaging.mail_search }"/>
			        	<c:forEach var="i" begin="${mailpaging.begin }" end="${mailpaging.end }">
			            	<c:if test="${mail_search eq ''}">
			                  <a href="${cpath }/mail/receiveList?page=${i}">[${i }]</a>                  
			               	</c:if>
			               	<c:if test="${mail_search ne ''}">
			                	<a href="${cpath }/mail/receiveList?page=${i}&mail_search=${mailpaging.mail_search}&mail_column=${mailpaging.mail_column}">[${i }]</a>
			                </c:if>   
			            </c:forEach>
			               
			            <c:if test="${mailpaging.next }">
			            	<a href="${cpath }/mail/receiveList?page=${mailpaging.end + 1}&mail_search=${mailpaging.mail_search}&mail_column=${mailpaging.mail_column}">[다음]</a>
			            </c:if>
					</div>
				</div>
    		...
    		if(check===true){
	    		$.ajax({
	    			...
	    			success: function(jdata){
	    				if(jdata = 1){
	    					alert("삭제성공");
	    					location.replace("${cpath}/mail/receiveList")
	    				}
	    				else{
	    					alert("삭제실패");
	    				}
