...
<link rel="stylesheet" type="text/css" href ="${cpath }/resources/css/mail/sendList.css">
</head>
<body>
...
<div id="homeMain">
	<div id="mailMainBox">
		<div id="mailSideBox">
			...
			<form id="mail_column" method="POST" action="${cpath }/mail/sendList">
				<div class="mailSearchBox">
					<input type="text" id="mail_send_inputSearch" name="mail_search" placeholder="검색어를 입력하세요" >
					<input type="submit" name="mail_submit" value="검색">
				</div>
				<select id="mail_send_select" name="mail_column">
		...
		<div id="sendMailBox">
		        	...
		        	<tbody>
		        		<c:forEach var="dto" items="${list}">
			    			<tr>
			        			...
			        			<td><a href="${cpath}/mail/selectone/sender/${dto.mail_idx}">${dto.mail_title}</a></td>
          ...
          <div id="mail_foot">
              <div id="mail_paging">
                  ...
                  <a href="${cpath }/mail/sendList?page=${mailpaging.begin - mailpaging.perPage}&mail_search=${mailpaging.mail_search}&mail_column=${mailpaging.mail_column}">[이전]</a>
               </c:if>
               ...
               <c:forEach var="i" begin="${mailpaging.begin }" end="${mailpaging.end }">
                  <c:if test="${mail_search eq ''}">
                     <a href="${cpath }/mail/sendList?page=${i}">[${i }]</a>                  
                  </c:if>
                  <c:if test="${mail_search ne ''}">
                     <a href="${cpath }/mail/sendList?page=${i}&mail_search=${mailpaging.mail_search}&mail_column=${mailpaging.mail_column}">[${i }]</a>
                  </c:if>   
               </c:forEach>
               
               <c:if test="${mailpaging.next }">
                  <a href="${cpath }/mail/sendList?page=${mailpaging.end + 1}&mail_search=${mailpaging.mail_search}&mail_column=${mailpaging.mail_column}">[다음]</a>
               </c:if>
               </div>
            </div>
    	...
    	if(valueArr.length == 0){
    		...
    		$.ajax({
    			...
    			success: function(jdata){
    				if(jdata = 1){
    					alert("삭제성공");
    					location.replace("${cpath}/mail/sendList")
    				}
    				else{
    					alert("삭제실패");
    				}
